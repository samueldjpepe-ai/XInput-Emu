...
struct Mapeo {
    ...
    int deadzoneIZQ; 
    int deadzoneDER; 
    bool invertirX;  // <-- NUEVO
    bool invertirY;  // <-- NUEVO
};
...
// PROCESADOR CON INTERRUPTORES DE INVERSIÓN
short aplicarEjeFinal(int valorActual, int signoCalibrado, int deadzone, bool esEjeY, bool invX, bool invY) {
    ...
    float resultado;
    if (esEjeY) {
        // Estándar Xbox: Arriba es Negativo.
        resultado = -normalizado; 
        if (invY) resultado = -resultado; // Inversión manual del usuario
    } else {
        // Estándar Xbox: Derecha es Positivo.
        resultado = normalizado;
        if (invX) resultado = -resultado; // Inversión manual del usuario
    }
...
void calibrarMando(SDL_Joystick* joy, std::string guid) {
    Mapeo nuevo;
    system("cls");
    std::cout << "=== CONFIGURACION MANUAL DE MANDO ===\n";
    
    // 1. Zonas Muertas
    std::cout << "Zona muerta IZQ (1-100): "; std::cin >> nuevo.deadzoneIZQ;
    std::cout << "Zona muerta DER (1-100): "; std::cin >> nuevo.deadzoneDER;

    // 2. Preguntas de Inversión
    char res;
    std::cout << "¿Quieres INVERTIR el eje X (Izquierda/Derecha)? (s/n): "; std::cin >> res;
    nuevo.invertirX = (res == 's' || res == 'S');
    
    std::cout << "¿Quieres INVERTIR el eje Y (Arriba/Abajo)? (s/n): "; std::cin >> res;
    nuevo.invertirY = (res == 's' || res == 'S');

    // 3. Botones (Igual que antes)
    for (int i = 0; i < 10; i++) {
    ...
    guardarConfig();
    std::cout << "\n¡Mando guardado! Pulsa 0 para jugar y probar.\n";
    Sleep(1500);
}
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        std::cout << "=== EMULADOR PRO XINPUT ===\n";
        std::vector<std::string> gList;
        for (int i = 0; i < n && i < 6; i++) {
            SDL_JoystickOpen(i);
            char gs[33]; SDL_JoystickGetGUIDString(SDL_JoystickGetGUID(SDL_JoystickOpen(i)), gs, 33);
            gList.push_back(gs);
            std::cout << "[" << i + 1 << "] " << SDL_JoystickName(SDL_JoystickOpen(i)) << (baseDeDatos.count(gs) ? " [LISTO]" : " [!] CONFIGURAR") << std::endl;
        }

        std::cout << "\n[0] JUGAR | [1-6] Configurar/Invertir | [ESC] Salir\nOpcion: ";
        int op; std::cin >> op;
        ...
        if (op == 0) {
            ...
            std::cout << "EMULANDO... (Presiona 'M' en el teclado para volver al menu)\n";
            while (true) {
