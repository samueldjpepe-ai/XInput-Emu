...
// ============================================================
// FUNCIÓN CORREGIDA PARA LOS EJES
// ============================================================
...
short procesarEjeXbox(
    ...
    // Deadzone en porcentaje -> rango SDL
    ...
    // Normalizar dirección según calibración
    if (signoCalibrado < 0)
        valor = -valor;
...
void calibrarMando(SDL_Joystick* joy, std::string guid) {
    ...
    std::cout << "Zona muerta IZQ (1-100): ";
    std::cin >> nuevo.deadzoneIZQ;

    std::cout << "Zona muerta DER (1-100): ";
        ...
        while (!cap) {
            ...
            while (SDL_PollEvent(&ev)) {
                ...
                if (ev.type == SDL_JOYBUTTONDOWN &&
                    ev.jbutton.which == SDL_JoystickInstanceID(joy)) {
        ...
        if (op == 0) {
            ...
            while (true) {
                ...
                for (auto& e : emus) {
                    ...
                    // Stick izquierdo
                    r.sThumbLY = procesarEjeXbox(
                        SDL_JoystickGetAxis(e.physical, m.ejes[0]),
                        m.signos[0],
                        m.deadzoneIZQ,
                        true
                    );

                    r.sThumbLX = procesarEjeXbox(
                        SDL_JoystickGetAxis(e.physical, m.ejes[1]),
                        m.signos[1],
                        m.deadzoneIZQ,
                        false
                    );
                    ...
                    // Stick derecho
                    r.sThumbRY = procesarEjeXbox(
                        SDL_JoystickGetAxis(e.physical, m.ejes[2]),
                        m.signos[2],
                        m.deadzoneDER,
                        true
                    );

                    r.sThumbRX = procesarEjeXbox(
                        SDL_JoystickGetAxis(e.physical, m.ejes[3]),
                        m.signos[3],
                        m.deadzoneDER,
                        false
                    );
                    ...
                    if (SDL_JoystickGetButton(e.physical, 6))
                        r.bLeftTrigger = 255;

                    if (SDL_JoystickGetButton(e.physical, 7))
                        r.bRightTrigger = 255;
